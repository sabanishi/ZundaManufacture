...
namespace Sabanishi.ZundaManufacture.Editor
{
    public class LayerNameCreator
    {
        ...
        private const string PathName = "Assets/Scripts/Common/LayerName.cs";
        ...
        [MenuItem("Tools/Create/LayerName")]
        private static void Open()
        {
            if (!CanCreate())
            {
                //警告ポップアップを表示
                EditorUtility.DisplayDialog("LayerNameCreator[失敗]", "現在作成できません(Playモード/コンパイル中)", "OK");
                return;
            }
            ...
            //完了ポップアップを表示
            EditorUtility.DisplayDialog("LayerNameCreator[成功]", "作成が完了しました", "OK");
        }
        ...
        private static void Create()
        {
            ...
            AppendLine(1,"/// レイヤー名を定数で管理するクラス", builder);
            AppendLine(1,"/// </summary>", builder);
            
            AppendLine(1, "public static class LayerName", builder);
            AppendLine(1, "{", builder);

            foreach (var n in InternalEditorUtility.layers.Select(c => new { left = RemoveInvalidChars(c), right = c }))
            {
